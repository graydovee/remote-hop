...
8. 如果 jumpserver 要求 MFA 且未配置 TOTP secret，CLI 会提示输入一次性验证码。
9. 同一逻辑目标有空闲连接时优先复用。
...
这里 `rhop` 会把 `foo-10-92-1-163` 推导成 `10.92.1.163`，然后按 `server.toml -> fallback` 的顺序解析目标；如果 daemon 没启动则自动启动，然后执行远端命令。
